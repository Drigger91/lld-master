...
- Access levels are strictly ordered (`NO_ACCESS < READ < EDIT < OWNER`) via a numeric priority, so "can edit" is `level >= EDIT`.
...
## Run
mvn -q -pl problems/document-sharing-service compile exec:java
